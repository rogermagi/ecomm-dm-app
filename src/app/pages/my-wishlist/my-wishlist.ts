  ...
  <div class="mx-auto max-w-[1200px] py-6 px-4">
      ...
      @if(store.wishlistCount() >0) {
        <div class="flex justify-between items-center mb-6">
          ...
          <span class="text-gray-500 text-xl">{{store.wishlistCount()}} Items found</span>
        </div>
        ...
        <div class="responsive-grid">
          @for(product of store.wishlistItems(); track product.id) {
            <app-product-card [product]="product">
                <button 
                class="!absolute z-10 top-3 right-3 !bg-white transition-all duration-200 hover:scale-110 hover:shadow-lg" 
                matIconButton 
